...
async def reprise_projet(req: RepriseRequest):
    ...
    if not projet:
        raise HTTPException(
            status_code=400, detail="Le champ 'project' ne peut pas être vide."
        )
    ...
    for script_path in (
        ...
        if not script_path.exists():
            return RepriseResponse(
                status="error", detail=f"Script manquant : {script_path.name}"
            )
    ...
    try:
        ...
        return RepriseResponse(
            status="error", detail=f"Échec discord_fetcher.py : {e.stderr or e}"
        )
    ...
    try:
        ...
        return RepriseResponse(
            status="error", detail=f"Échec project_resume.py : {e.stderr or e}"
        )
    ...
    if not resume_folder.exists():
        return RepriseResponse(
            status="error", detail=f"Aucun dossier 'resume' pour le projet {projet}."
        )

    all_resumes = sorted(
        resume_folder.glob("resume_gpt_*.md"), key=lambda f: f.stat().st_mtime
    )
    if not all_resumes:
        return RepriseResponse(
            status="error", detail="Aucun fichier 'resume_gpt_*.md' trouvé."
        )
    ...
    return RepriseResponse(
        status="success", resume_path=str(last_resume), resume_content=content
    )
...
async def write_note(req: WriteNoteRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Échec d’écriture de la note : {repr(e)}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Note ajoutée au JSON, mais échec du journal Markdown : {repr(e)}",
        )
    ...
    if req.project:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Échec écriture memoire_{project_slug}.md : {repr(e)}",
            )
...
async def get_notes():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Impossible de lire le fichier : {repr(e)}"
        )
...
async def read_note(term: str = "", limit: int = 5):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur lecture mémoire : {repr(e)}"
        )
...
async def delete_file(req: DeleteFileRequest):
    ...
    try:
        ...
        return WriteResponse(
            status="error", detail=f"Fichier introuvable : {file_path}"
        )
    except PermissionError as e:
        return WriteResponse(status="error", detail=f"Permission refusée : {e}")
    except Exception as e:
        return WriteResponse(
            status="error", detail=f"Erreur suppression du fichier {file_path} : {e}"
        )
    ...
    return WriteResponse(
        status="success", detail=f"Fichier supprimé : {file_path}", path=str(file_path)
    )
...
async def move_file(req: MoveFileRequest):
    ...
    try:
        ...
        return WriteResponse(
            status="error", detail=f"Fichier source introuvable : {src_path}"
        )
    except PermissionError as e:
        return WriteResponse(status="error", detail=f"Permission refusée : {e}")
    except Exception as e:
        return WriteResponse(
            status="error", detail=f"Erreur déplacement {src_path} -> {dst_path} : {e}"
        )
    ...
    return WriteResponse(
        status="success", detail=f"Fichier déplacé : {dst_path}", path=str(dst_path)
    )
...
async def archive_file(req: ArchiveFileRequest):
    ...
    try:
        ...
        return WriteResponse(
            status="error", detail=f"Impossible de créer le dossier {archive_dir} : {e}"
        )
    ...
    try:
        ...
        return WriteResponse(
            status="error", detail=f"Erreur archivage {src_path} : {e}"
        )
    ...
    return WriteResponse(
        status="success", detail=f"Fichier archivé : {dest_path}", path=str(dest_path)
    )
...
async def home():
    return JSONResponse(
        content={
            "message": "✅ API SENTRA_CORE_MEM active. Bienvenue sur le noyau IA local."
        }
    )
