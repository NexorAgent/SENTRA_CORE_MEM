...
# ————————————————
# 3. Appel à l’API (interface adaptée selon la version)
# ————————————————
...
def main(project_name: str):
    ...
    # Prendre le fichier de résumé le plus récemment modifié
    ...
    # Configurer OpenAI et résumer
    setup_openai()
    summary = summarize_with_gpt(glyph)

    # Sauvegarder le résumé GPT
    output_folder = Path("projects") / project_slug / "resume"
    output_path = output_folder / f"resume_gpt_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    try:
